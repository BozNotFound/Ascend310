...
//     e=buff[2];
//     char s[20];
//     gcvt(a, 6, s);  // 转换为字符串，保留6位有效数字Sht20Close();
//     char s1[20];
//    gcvt(d, 6, s1);  // 转换为字符串，保留6位有效数字Sht20Close();
//    char s2[20];
//    gcvt(e, 6, s2);  // 转换为字符串，保留6位有效数字Sht20Close();
...
//    gcvt(e, 6, s2);  // 转换为字符串，保留6位有效数字Sht20Close();
    ...
    // char s[20];
    // gcvt(b, 6, s);  // 转换为字符串，保留6位有效数字Sht20Close();
    // char s1[20];
    // gcvt(c, 6, s1);  // 转换为字符串，保留6位有效数字Sht20Close();
    a=buff[0];
    char s[20];
    gcvt(a, 6, s);  // 转换为字符串，保留6位有效数字Sht20Close();
    ColorTurn(TURNOVER_COLOR);//反色显示
    DisplayTurn(NORMAL_DISPLAY);//正常显示
    ShowString(0,0,s,size1206);//显示ASCII字符 
  //  ShowString(0,12,s1,size1608);//显示ASCII字符    
  //  ShowString(0,28,s2,size1608);//显示ASCII字符  
    Refresh();
   usleep(time);//液晶屏显示
