...
{   
    ...
    ret = Spi_Init(&(adxl345->spi),"/dev/spidev0.1","adxl345",SPI_MODE_3,8,1000000,436);
    if(ret)
    {
        printf("adxl345 device init error\n");
        return -1;
    }
    adxl345_read_Cmd(0x00,&devid);
    if( devid !=0xE5)
    {
        printf("[ERROR] read device id error\n");
    }
    ...
    adxl345_write_Cmd(ADXL345_REG_BW_RATE,0x0A);    /* rate 3200Hz*/
    adxl345_write_Cmd(ADXL345_REG_INT_ENABLE,0x00);
    adxl345_write_Cmd(ADXL345_REG_INT_MAP,0x80);
